...
| **二 · 组合** | 05–08 | 原语一起工作。失败长什么样、身份怎么表达、Host 状态怎么观察、外部代码怎么进来 |
| **三 · 完整应用** | 09–12 | 前八章的东西排成真实应用的形状，**不引入任何新原语** |
...
| **02** | ExtensionPoint + Event | `extension-point` `contribute` `contribution-view` `event` `contribution-dispose` | ExtensionPoint 保存当前贡献，Event 什么都不留——它不是查询 API |
...
| **06** | Contract family 与 Group | `contract-family` `group` `atomic-commit` `group-removal` | 同一能力的多个静态变体用显式 Contract family；Group 只表达安装所有权 |
...
答案是 Dougong 最有辨识度的语义，也是它的主要取舍：一笔 ChangeSet 里任何一个 Installation 激活失败，整笔回滚。示例里的 audit Instance **确实启动了**，然后被撤销——`started 1 time and was released 1 time`。
...
| # | 场景 | 新增概念 | 它证明了什么 |
| --- | --- | --- | --- |
| **09** | Planet | `call-time-selection` `live-provider-swap` `group-bound-platform` | Provider 增删不重启 Player——ExtensionPoint 不是依赖边 |
| **10** | Lynx | `domain-catalog` `workspace-ownership` `registration-update` | 命令唯一性是领域策略；根级消费者能看到 Group 的贡献，Group 不是 Scope |
| **11** | 声明式计划 | `desired-state` `content-revision` `platform-change-set` | 期望状态 diff 成一份 ChangeSet；身份来自 Manifest name，变化来自显式 revision |
| **12** | HMR 模块图 | `module-graph` `invalidation-closure` `multi-registration-hmr` | 沿 importer 方向传播失效；两个 Registration 原子更新，观察者只看到 1 次提交 |
